...
use ModelSupports\validators\StringValidator;
...
class Html
{
    const ID_PREFIX = 'pf_'; // HTML-TAG 的id前缀
    ...
    static protected function normalizeUrl($url)
    {
        if (is_array($url)) {
            if (isset($url[0])) {
                return UrlManager::getInstance('url-manager')->createUrl($url[0], array_splice($url, 1));
            } else {
                return Request::httpRequest()->getUrl();
            }
        } else if (!empty($url)) {
            return $url;
        }
     ...
     * @return string
     */
    static public function label($text, $for = false, $htmlOptions = [])
    {
        if (false === $for) {
    ...
    static public function refresh($seconds = 3, $url = '')
    {
        ...
        ClientScript::getInstance()->registerMetaTag($content, null, 'refresh');
    }
     ...
     * @return string
     */
    static public function fileField($name, $htmlOptions = [])
    {
        return self::inputField('file', $name, null, $htmlOptions);
    }
